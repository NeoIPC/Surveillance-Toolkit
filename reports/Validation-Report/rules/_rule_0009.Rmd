```{r Validation rule 9: HAP event not completed on completed enrolment/end}
problems <- problems |>
  dplyr::bind_rows(
    data$enrollments |>
      ...
      dplyr::anti_join(
        exceptions |> dplyr::filter(.data$rule_id == 9L),
        dplyr::join_by("event_key")) |>
      tidyr::nest(context = c("enrollment_status", "end_status", "hap_status")) |>
      dplyr::mutate(
        rule_id        = 9L,
