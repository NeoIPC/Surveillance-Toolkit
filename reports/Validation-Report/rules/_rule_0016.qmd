```{r Validation rule 16: "The patient record contains a SSI form with an infection date that is not within the time frame of a documented enrolment."}
...
problems <- problems |>
  bind_rows(
    patients |>
      inner_join(
        enrollments |>
          inner_join(data$ssis, join_by(key == enrollment)),
        join_by(key == patient)) |>
