```{r Validation rule 11: SSI event not completed on completed enrolment/end}
problems <- problems |>
  dplyr::bind_rows(
    data$enrollments |>
      ...
      dplyr::anti_join(
        exceptions |> dplyr::filter(.data$rule_id == 11L),
        dplyr::join_by("event_key")) |>
      tidyr::nest(context = c("enrollment_status", "end_status", "ssi_status")) |>
      dplyr::mutate(
        rule_id        = 11L,
