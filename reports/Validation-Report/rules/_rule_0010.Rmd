```{r Validation rule 10: surgery event not completed on completed enrolment/end}
problems <- problems |>
  dplyr::bind_rows(
    data$enrollments |>
      ...
      dplyr::anti_join(
        exceptions |> dplyr::filter(.data$rule_id == 10L),
        dplyr::join_by("event_key")) |>
      tidyr::nest(context = c("enrollment_status", "end_status", "pro_status")) |>
      dplyr::mutate(
        rule_id        = 10L,
