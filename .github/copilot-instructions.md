...
- **Always** treat accessibility and long-term archivability of generated user-visible or publicly published documents as explicit, first-class goals of the NeoIPC Surveillance Toolkit — not optional polish. Wherever a NeoIPC tool produces a document a person reads or that is published (reports, certificates, web UIs), enable and maintain **both**: accessibility conformance (WCAG via `axe` on HTML, PDF/UA tagged PDF with alt text on every figure and decorative images marked as artifacts, accessible web UIs) and long-term archival compliance (PDF/A). When adding or changing such output, keep these properties working and do not regress them; prefer formats and settings that preserve them, and treat a loss of tagging/alt-text/PDF-A conformance as a defect.
...
**never** response bodies, a data-protection boundary). The `Build-*.ps1` wrappers map the standard
