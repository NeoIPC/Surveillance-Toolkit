...
- **Never** drop a pathogen name from `NeoIPC-Infectious-Agents.yaml` when it is renamed or reclassified — retain the prior name as a **synonym** of the current concept, **keeping its original `Id`** so values already entered against that name still resolve. The `Id` is the DHIS2 option-code stored in collected surveillance data, so it must follow the name as it becomes a synonym — do **not** mint a new `Id` for the demoted name or retire the old one. (New concepts get the next free `Id` = `max+1`, gaps not refilled; a genuinely retired `Id` is never reassigned to a *different* organism.) Whole-branch moves (mostly in viruses) can complicate this, but bacterial renames are normally clean per-name retentions. *(repo-specific)*
