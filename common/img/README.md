...
Colours are the brand's two, **`#0083c1`** and **`#ff9015`**, applied through the classes `brand-blue`
and `brand-orange`. The converted artwork carried `#0083c2` / `#ff9016`, a rounding artifact of the
colour conversion, normalised to the exact brand values rather than kept.
...
each as a fraction of the mark's width. The spacing between the logo's elements and their proportions are
not ours to alter, so neither figure is a design decision.
...
Each lockup has a minimum size below which it is not legible, and generated sheets place the horizontal
one comfortably above it, at 42 mm. The minima are a brand requirement rather than a typographic
judgement — check them before changing `LOGO_W` in `scripts/build-collection-sheets.py`.
